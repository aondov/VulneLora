...
# TODO: include testing the connection to injected DB
...
def argument_parser():
    ...
    while True:
        ...
        if arg_input == "help":
            with open(service_path + '/modes/help_messages/sql_injection.txt', 'r') as file:
                file_content = file.read()
                print(file_content)
        elif "payload" in arg_input:
            tmp_payload = extract(arg_input, "payload")
            if len(tmp_payload) < 255:
                attack_config['payload'] = tmp_payload
                print(f"\n>> Set argument: payload={tmp_payload}")
            else:
                attack_config['payload'] = ""
                print(f"\n>> Set argument: payload='' (default value, payload needs to have less than 255 characters)")
...
def run_attack():
    ...
    subprocess.run(["platformio", "run", "-d", "/opt/vulnelora/resources/LoMAB/"])
    subprocess.run(["platformio", "run", "-d", "/opt/vulnelora/resources/LoMAB/", "-t", "upload"])


if __name__ == "__main__":
    try:
        # result = subprocess.run(["dmesg", "|", "grep", "-A", "5", "-B", "5", "FT232R", "|", "grep", "-o", "ttyUSB.", "|", "uniq"], capture_output=True, text=True, shell=True)
        # print(result.stdout)
        # exit(0)
        argument_parser()
        run_attack()
