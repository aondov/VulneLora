...
def argument_parser():
    ...
    while True:
        ...
        if arg_input == "help":
            ...
            tmp_s_capture = extract(arg_input, "save_capture")
            if tmp_s_capture:
                attack_config['save_capture'] = True
                print(f"\n>> Enabled argument: save_capture")
            elif not tmp_s_capture:
                attack_config['save_capture'] = False
                print(f"\n>> Disabled argument: save_capture")
            else:
                attack_config['save_capture'] = False
                print(f"\n>> Disabled argument: save_capture (revert to default value, can be only True or False)")
        elif "save_analysis" in arg_input:
            tmp_s_analysis = extract(arg_input, "save_analysis")
            if tmp_s_analysis:
                attack_config['save_analysis'] = True
                print(f"\n>> Enabled argument: save_analysis")
            elif not tmp_s_analysis:
                attack_config['save_analysis'] = False
                print(f"\n>> Disabled argument: save_analysis")
            else:
                attack_config['save_analysis'] = False
                print(f"\n>> Disabled argument: save_analysis (revert to default value, can be only True or False)")
